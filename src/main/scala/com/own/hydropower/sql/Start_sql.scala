...
object Start_sql {
  ...
  private val log = LogManager.getLogger(Start_sql.getClass)
  ...
  def dataToHdfs(sparkSession: SparkSession): Unit = {
    ...
    val sql = "insert overwrite directory " + "'" + Collection.hdfsPath + Collection.businessType + date + "/" + "'" +
      " ROW FORMAT DELIMITED FIELDS TERMINATED BY '\\t' select * from " + "default." + Collection.hdfsTable
    println("tohdfs:  " + sql)
    sparkSession.sql(sql)
    val cmd = "hadoop fs -rm -f " + Collection.hdfsPath + Collection.businessType + date + "/_SUCCESS"
    println("cmd: " + cmd)
